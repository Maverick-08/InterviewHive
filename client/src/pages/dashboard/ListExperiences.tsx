...
const ListExperiences = ({
  ...
  const handlePageIncrement = () => {
    if(!isLoading && (page+1)<=(totalCount/10)+1){
      setPage(page+1);
    }
  };

  const handlePageDecrement = () => {
    if (!isLoading && page-1 > 0) {
      setPage(page - 1);
    }
  };
    ...
    <div className="flex flex-col gap-12">
        ...
        {interviewData.map((data, index) => {
          return (
            <div
              ...
              className={cn(`px-4 py-2 flex flex-col bg-[#171717] border-1 border-[#333333] rounded-md  cursor-pointer ${isModalOpen ? '':'hover:scale-105 ease-in duration-200'}`)}
            >
      ...
      <div className="flex justify-center">
        <div className="flex gap-12 items-center">
          ...
          {/* previous page  */}
          <div onClick={handlePageDecrement} className={cn(`flex items-center gap-2 ${!isLoading && !(page-1 > 0) ? 'text-neutral-500':'text-zinc-300 hover:scale-115'}  cursor-pointer select-none ease-in duration-300`)}>
            <MdKeyboardDoubleArrowLeft className="h-6 w-6" />
            <span className="text-lg">Previous</span>
          </div>

          {/* next page  */}
          <div onClick={handlePageIncrement} className={cn(`flex items-center gap-2 ${!isLoading && !((page+1)<=(totalCount/10)+1) ? 'text-neutral-500':'text-zinc-300 hover:scale-115'}  cursor-pointer select-none ease-in duration-300`)}>
