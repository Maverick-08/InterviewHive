...
const Message = ({
  ...
  return (
    ...
    >
      <span
        className={cn(`text-sm py-1 px-2 rounded-md break-words text-balance ${
          sentBy == "user" ? "w-fit bg-amber-300/90 text-black" : "bg-neutral-700/40 w-[70%] "
        }`)}
      >
        {sentBy == "user" ? message : <TypingText text={message} repeat={false} hideCursorOnComplete={true} grow={true} className="leading-6" /> }
      </span>
    </div>
  );
};
